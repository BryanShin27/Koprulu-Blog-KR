...
지금까지 저희는 별도의 수익 창출 없이 열정과 낭만을 동력으로 개인의 시간과 비용을 들여가며 힘겹게 모드를 개발해 왔습니다. 저희 모드에 있는 고유 모델링이나 애니메이션은 모두 일정 수준 이상의 퀄리티를 기준점으로 하는 전문적 3D 작업을 거쳐 완성되었으며, 그밖에도 지속적인 게임 내부 데이터 관리 및 패치 등 모든 부분에 적지 않은 시간과 비용이 들어갔습니다. 
